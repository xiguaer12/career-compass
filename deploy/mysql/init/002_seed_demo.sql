...
VALUES
  ...
  ('学校邮箱自动识别学号', '公告', '注册时会根据学校邮箱前 10 位数字自动绑定学号，并自动推算基础届别背景。', '注册时会根据学校邮箱前 10 位数字自动绑定学号，并自动推算基础届别背景。', '后台维护', '', '已发布'),
  ('公开权威数据发布前均需人工审核', '公告', '新增数据源抓取候选不会直接公开，管理员审核通过后才进入前台展示。', '新增数据源抓取候选不会直接公开，管理员审核通过后才进入前台展示。', '后台维护', '', '已发布'),
  ('AI 访谈草稿会保存多久？', 'FAQ', 'AI 访谈素材至少保存 180 天，再次进入会恢复最近一次整理位置。', 'AI 访谈素材至少保存 180 天，再次进入会恢复最近一次整理位置。', '后台维护', '', '已发布'),
